...
# A New Method For Defining HTTP APIs in Haskell
...
```mermaid
flowchart LR
  A["Dynamic<br/>Ruby, Clojure"]:::dynamic --- B["Idiomatic<br/>Haskell"]:::idiomatic --- C["Dependently Typed<br/>Agda, Lean"]:::dependent
...
quadrantChart
    ...
    y-axis Opaque --- Introspectable
    quadrant-1 Complex but Safe
    quadrant-2 The Sweet Spot
    quadrant-3 Simple but Opaque
...
Okapi provides primitive codecs for describing HTTP requests and responses. The most basic request and response codecs are `Req.any` and `Res.any`. These codecs represent the most general HTTP request, and the most general HTTP response.
...
Okapi provides base codecs where the request method is fixed, so you don't have to start with `Req.any` and then modify it with a method tag. You can just start with the method.
