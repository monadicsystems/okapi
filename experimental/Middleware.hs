...
applyMiddlewares :: MonadHTTP m => [m Response -> m Response] -> m Response -> m Response
applyMiddlewares ms handler =
  Prelude.foldl (\handler m -> m handler) handler ms

addHeadersMiddleware :: MonadHTTP m => m Response -> m Response
addHeadersMiddleware handler = setHeaders [] <$> handler

prefixPathMiddeware :: MonadHTTP m => [Text] -> (m Response -> m Response)
prefixPathMiddeware prefix handler = path prefix >> handler

-- | TODO: Is this needed? Idea taken from OCaml Dream framework
scope :: MonadHTTP m => [Text] -> [m Response -> m Response] -> (m Response -> m Response)
