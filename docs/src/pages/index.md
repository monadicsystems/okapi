...
Okapi also exports the type constraint `MonadHTTP m`, which is the abstract interface of `ServerT m`.
...
myParser' :: (MonadHTTP m, MonadIO m, MonadReader AppConfig m) => m (Int, Char)
myParser' = do
...
myParser1 :: (MonadHTTP m, MonadIO m) => m Response
myParser1 = do
  logIO "Using handler 1"
  ...

myParser2 :: MonadHTTP m => m Response
myParser2 = ...
