...
todoAPI :: (MonadIO m, MonadHTTP m) => Connection -> m Response
todoAPI conn =
  home conn
    <|> getAllTodos conn
    <|> createTodoForm
...
home :: (MonadHTTP m, MonadIO m) => Connection -> m Response
home conn = do
  methodGET
  pathEnd
...
counter :: MonadHTTP m => m Response
counter = do
  methodGET
...
getAllTodos :: (MonadHTTP m, MonadIO m) => Connection -> m Response
getAllTodos conn = do
  methodGET
...
createTodoForm :: MonadHTTP m => m Response
createTodoForm = do
    methodGET
    pathParam @Text `is` "todos"
...
createTodo :: (MonadHTTP m, MonadIO m) => Connection -> m Response
createTodo conn = do
  methodPOST
  pathParam @Text `is` "todos"
  pathEnd
...
editTodoStatus :: (MonadHTTP m, MonadIO m) => Connection -> m Response
...
forgetTodo :: (MonadHTTP m, MonadIO m) => Connection -> m Response
forgetTodo conn = do
  methodDELETE
  pathParam @Text `is` "todos"
