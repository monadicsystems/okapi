...
-- | Implements RFC 9651, "Structured Field Values for HTTP".
module Okapi.HTTP.Structured (
    Structured,
    ParseError (..),
    parser,
    printer,
...
-- >>> import Okapi.HTTP.Structured.BareItem (BareItem, Token, token)
-- >>> import Data.ByteString (ByteString)
...
-- >>> let mixedRFCDict = frequency [(1, arbitrary), (3, printer (dict (Dictionary.member "a" (Item.bareItem (integer :: Leaf BareItem Integer)))) <$> (arbitrary :: Gen Integer))] :: Gen ByteString
...
type Structured :: Type -> Type -> Type
data Structured i o where
    SItem :: Tree Item a a -> Structured a a
    SList :: Tree List a a -> Structured a a
    SDict :: Tree Dictionary a a -> Structured a a

type instance Context Structured = ByteString
type instance Failure Structured = ParseError
...
--   'Okapi.HTTP.Request.Headers.fieldDict' sound:
--
-- >>> parser (dict (Dictionary.member "a" (Item.bareItem (integer :: Leaf BareItem Integer)))) "a=1"
-- (Right 1,"")
-- >>> parser (dict (Dictionary.member "a" (Item.bareItem (integer :: Leaf BareItem Integer)))) "a=1, b=2"
-- (Right 1,", b=2")
-- >>> parser (dict (Dictionary.member "b" (Item.bareItem (integer :: Leaf BareItem Integer)))) ", b=2"
-- (Right 2,"")
-- >>> printer (dict (Dictionary.member "a" (Item.bareItem (integer :: Leaf BareItem Integer)))) 1
-- "a=1"
--
-- prop> printParse parser printer (dict (Dictionary.member "a" (Item.bareItem (integer :: Leaf BareItem Integer)))) (n :: Integer)
--
-- Same insignificant-OWS caveat as 'item' — see there.
--
-- prop> forAll mixedRFCDict (\bs -> parsePrintOr discard parser printer (dict (Dictionary.member "a" (Item.bareItem (integer :: Leaf BareItem Integer)))) (stripOWS bs))
--
...
-- 'Okapi.HTTP.Request.Headers.fieldDict' is actually in.
--
...
dict :: Tree Dictionary a a -> Tree Structured a a
dict = Node . SDict

parser :: Tree Structured i o -> Parser Structured o
parser = Tree.parser alg
  where
    alg :: Structured i o -> Parser Structured o
...
printer :: Tree Structured i o -> Printer Structured i
printer = Tree.printer alg
  where
    alg :: Structured i o -> Printer Structured i
...
parseExact :: Tree Structured i o -> ByteString -> Either (Either ParseError ByteString) o
