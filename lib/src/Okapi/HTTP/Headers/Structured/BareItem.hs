...
module Okapi.HTTP.Headers.Structured.BareItem (
    BareItem,
    renderInner,
    parseInnerToList,
) where
...
import Okapi.Leaf (ErrorOf, HasLeaf (..), Info (..), Leaf (..), PieceOf)
import Okapi.HTTP.Headers.Structured.Lexer (ParseError (..), eParse, parseInnerBare, strip)

data BareItem a

type instance PieceOf BareItem = ByteString
type instance ErrorOf BareItem = ParseError
