...
fetch ::
    HC.Manager ->
    String ->
    Contract (Signature method path query headers body result) ->
    Req.Data method path query headers body ->
...
{- | Heterogeneous-@n@ counterpart to 'client' — takes a record built with
  'Okapi.Contract.Morph' (see 'Okapi.Server.serversVia') instead
